...
本仓库的开发、迭代、重构与修复默认遵循“分阶段完成即提交并推送”的闭环流程。

- 每个需求都要先拆成符合功能逻辑的阶段。
- 每完成一个阶段，都要先验证，再单独提交，再推送到 GitHub，然后继续下一个阶段。
- 不允许把多个里程碑揉成一次总提交，也不允许停留在“只做计划、不继续执行”的半完成状态。

完整规则见：[docs/development-workflow.md](./docs/development-workflow.md)
